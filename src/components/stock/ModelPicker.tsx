...
type Props = { value: CatalogModel | null; onChange: (next: CatalogModel | null) => void };
...
export default function ModelPicker({ value, onChange }: Props) {
  ...
  const results = useMemo(() => {
    if (!query.trim()) return CATALOG_MODELS.slice(0, 12);
  ...
  return (
    <div className="relative">
      {value && !focused ? (
        <button type="button" onClick={() => { setFocused(true); setQuery(""); }}
          className="ease-expo flex w-full items-center gap-3 rounded-md px-3 py-2.5 transition-colors hover:bg-white/[0.025]"
          style={{ background: "rgba(255,255,255,0.02)", boxShadow: "inset 0 0 0 1px rgba(59,130,246,0.3)" }}>
        ...
        <div className="flex items-center gap-2 rounded-md px-3 py-2.5" style={{ background: "rgba(255,255,255,0.02)", boxShadow: "inset 0 0 0 1px rgba(255,255,255,0.06)" }}>
      ...
      {focused && (
        <div className="absolute left-0 right-0 top-full z-30 mt-1 max-h-[280px] overflow-y-auto rounded-md py-1" style={{ background: "var(--mk-card-deep, #18181B)", boxShadow: "0 8px 24px rgba(0,0,0,0.4), inset 0 0 0 1px rgba(255,255,255,0.06)" }}>
          {results.length === 0 && (<div className="px-3 py-3 text-[12px] text-zinc-500">Aucun modèle trouvé.</div>)}
          {results.map((m) => (
            <button key={m.id} type="button" onMouseDown={(e) => { e.preventDefault(); onChange(m); setQuery(""); setFocused(false); }}
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="font-mono text-[10px] tracking-[0.1em] text-zinc-500">{m.category} · {m.manufacturer}</div>
              </div>
              <div className="font-mono text-[12px] tabular-nums text-zinc-400">{m.median_eur} €</div>
            </button>
          ))}
        </div>
      )}
